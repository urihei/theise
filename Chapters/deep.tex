...
Note that a linear classifier in $\Re^6$ induce non-linear classifier in $\Re^2$, in our example the classifier $f = \deltaF{x_1x_2 < 1}$ is clearly linear in $\phi([x_1, x_2])$ while a circle in $\Re^2$.
Define the polynomial kernel as $K_{c,d}(\xx,\xx') = (\xx \cdot \xx'+c)^d$.
It is easy to see that $K_{c,2}(\xx, \xx') \approx \phi([x_1, x_2]) \cdot \phi([x'_1, x'_2])$\footnote{For equality $\phi([x_1, x_2]) = [x_1^2, x_2^2, \sqrt{2}x_1x_2, \sqrt{2c}x_1, \sqrt{2c}x_2, \sqrt{c}]$}.
The learned classifier by SVM will have the form of $\deltaF{f(\xx) >0}$ where $f(\xx) = \sum_{i=1}^N \alpha_{i}K(\xx,\xx^i)$ \footnote{One big draw back of the kernel method is the dependence of the classification time complexity in the sample size $N$. Moreover, all samples with non-zero $\alpha_i$ needed to be saved as a part of the classifier.}.
...
Using kernel in the dot product allow us to change the hypothesis class from linear function in the original space $\Re^p$ to linear classifier in the reproduced Hilbert space.
