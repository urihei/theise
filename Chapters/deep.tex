...
Classification is one of the most fundamental machine learning problems. The goal of a classifier is simply to assign a label to an object. Assume there are $R$ possible labels (e.g., when classifying digits $R=10$). We will consider classifiers specified as follows. Given an input object $\xx \in \Re^\nvars$ , and a set of parameters $\ww\in\Re^q$, the classifier is a function $f: \Re^\nvars \times \Re^q \to \Re^R$. The output label for an input $\xx$ is given simply by
%%U
\footnote{Without loss of generality we assume that the labels are $y \in 1,\ldots,R$}:
%%
\be
y = \arg\max_{r\in 1,\ldots, R} f_r(\xx,\ww)
...
Assume we are given $\nsamples$ samples $D = \{(\xx^i,y^i)\}_{i=1}^\nsamples$  where $\xx^i \in \Re^\nvars, y^i \in 1,\ldots,R$ sampled IID from some fixed distribution $P$.
...
The parameters $\ww$ will be a set of $R$ vectors $\ww_i \in \Re^q$, and $f_r$ is defined as:
\be
...
\be
\label{eq:svm_obj}
\min_{\ww \in \Re^{R,q}} \sum_{i=1}^\nsamples \max_{r}\{\deltaF{r \ne y} - \ww_{y^i} \cdot \phi(\xx^{i}) + \ww_r \cdot \phi(\xx^i)\} + \lambda \|\ww\|^2
\ee
Since this is a convex optimization problem, it has a convex dual \cite{boyd2004convex} whose solution also yields $\ww$. The dual variables in this case are vectors $\alphav_i\in\Re^R$ (for $i=1,\ldots,\nsamples$)
...
Finally, we note that kernels can be used to capture certain invariants of the input space, a goal shared by neural architectures. For such approaches see \cite{bruna2013invariant}.\atodo{cite Chris Bishop, scattering network and  other kernel work by Bruna - I am not sure what you mean}.
