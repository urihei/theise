...
\todo{Also not clear.}
%In order to use projection gradient the input to the learning algorithm must be the same cardinality as the output  - no loss of information
%%
...
Moreover, it can be shown that $\bethelikelihood{X; \thetav}$ is \textbf{strictly} convex.\atodo{cite Stephen Bach who shows this - did not find this citation can you be more specific.(proved it in the past)} % (see \appref{})
...
\todo{Not sure what this means in the current context. Is this what Ganapathi does? - Yes this is how he calculate the Bethe marginals}
...
\atodo{You didn't say anything about consistency, unless I missed something and we now have it twice. Please check. - I did not understand what you mean.}
...
Similarly to Hammersley Clifford theorem, it can be proved that any positive Gibbs distribution with pairwise interaction can be express as $P(\xx;\thetav) = P(\tilde{\xx}) \exp{\sum_{ij} \theta^{\dagger}_{ij}(x_i,x_j) + \sum_{i}\theta^{\dagger}_i(x_i)}$.
