...
Contrastive Divergance (CD) \cite{hinton2002training} was designed for models where some of the variables are unobserved, but it can also be used in the fully observed case. Recall from \eqref{eq:ml_derv_single} and \eqref{eq:ml_derv_pairs} that the reason gradient-descent is impractical is the hardness of evaluating the marginals according to the parameters $p(x_i,x_j;\thetav)$. 
