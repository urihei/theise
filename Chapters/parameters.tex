...
\todo{Not clear what the result is.} 
...
Moreover, it can be shown that $\bethelikelihood{X; \thetav}$ is \textbf{strictly} convex.\atodo{cite Stephen Bach who shows this - did not find this citation can you be more specific.(I proved it in the past)} % (see \appref{})
...
Then, to optimize the relaxed objective, the Bethe marginals must be found - which is achieved by \cite{yuille2002cccp}.
\todo{Not sure what this means in the current context. Is this what Ganapathi does? - Yes this is how he calculates the Bethe marginals}
...
\red{ Note that as ML, PL is consistent.}
\ignore{
...
\atodo{You didn't say anything about consistency, unless I missed something and we now have it twice. Please check.}
...
Using the ideas from the proof of the Hammersley Clifford theorem,  they estimate the canonical factors, hence learning the graph parameters.
%%U
More specifically, to simplify presentation I will focus on the pairwise case. Set a fix assignment $\tilde{\xx}$, the pairwise canonical factor is :
\bean
\label{eq:HC_param}
...
\eean
while $\theta^{\dagger}_i(x_i)$ follows in the same way. 
Similarly to Hammersley-Clifford theorem, it can be proven that any positive Gibbs distribution with pairwise interaction can be expressed as $P(\xx;\thetav) = P(\tilde{\xx}) \exp{\sum_{ij} \theta^{\dagger}_{ij}(x_i,x_j) + \sum_{i}\theta^{\dagger}_i(x_i)}$.
Using this fact for learning, the parameters can be estimated by using the empirical distribution in \eqref{eq:HC_param}.
Note the resemblance of  \eqref{eq:HC_param} to the PL equation.
\ignore{
More specifically, they consider non pairwise MRFs of the form:
\atodo{Not clear what $p(\tilde{x})$ is below. All this part needs to be explained more clearly.} 
\be
P(\xx;\exact{\thetav}) = \frac{1}{Z(\thetav)} \exp{\sum_{c \in \cC}\thetav_c(\xx_c)} = P(\tilde{\xx})\exp{\sum_{c\in\cC} \thetav^{\dagger}_c(\xx_c)}
\ee
Where $\tilde{\xx}$ is some fix assignment.
