...
s.t. \xx_k = x_k}} \frac{1}{Z(\thetav)}\exp{\sum_{ij \in E} \theta_{ij}(x_i,x_j) + \sum_{i \in V} \theta_i(x_i)} = p(x_i;\thetav)\\
...
Contrastive Divergance (CD) \cite{hinton2002training} is a method for parameters learning. 
It was designed for models where part of the variables are hidden ( variables that there are no samples include them), but it could be used to general models.
The idea is simple, remember \eqref{eq:ml_derv_single} and \eqref{eq:ml_derv_pairs} the difficulty is to evaluate the marginals according to the parameters $p(x_i,x_j;\thetav)$. 
One way to approximate this quantity is by MCMC \ref{}.
...
Justification to this method was given by \cite{bengio2009justifying} and \cite{carreira2005contrastive} but it success in learning Restricted Boltzmann Machine (RBM) gain her its importance.
 ...
 A closed form equation for parameters estimation was given by \cite{abbeel2006learning}.
...
\ee
where  $f(b,\xx_c) : b \times \cX^{|c|} \times \cX^{|d|}\to \cX^{|d|}$  such that $b \subseteq c\subseteq d$ and for all $i \in d$ if $i \in b$ than $x_i = [\xx_c]_i$ otherwise $x_i = \tilde{\xx_i}$ for the fixed assignment $\tilde{\xx}$.
...
\ee
So a factor involve only the cliques and the neighbored variables.
Hence to estimate the parameters we just need to change $p(f(b,\xx_c,\tilde{\xx}_c) |\tilde{\xx}_{\nei{c}})$ by corespondent empirical marginals.
...
Moreover a compression of the sample complexity of this method, PL(more accurately of the composite likelihoods)  and ML is given.
