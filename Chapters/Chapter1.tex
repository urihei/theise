...
The message is a function of the messages from all neighbors except the destination neighbor and the factor connecting the two vertices.
We continue to cycle until the difference in the messages is very small or after a fix number of cycles.
...
If we want to understand BP we need to give meaning to the resulted pseudo-marginals.
...
I will now try to explain this result,
first lets defined the Bethe entropy,
\bean
...
\eean
Now we need another approximation, the approximation of the space the optimization is over - replacing the marginal-polytope with the local-polytope.
\be
...
Note that we replaced a convex function - the real entropy with a non convex function - the Bethe entropy hence this optimization is no longer convex.
Moreover, I am not aware on any result of the complexity of this objective.
So the importance of \eqref{eq:bethe_approximation} to inference, is its connection to BP.

To see the connection to BP we just need to write the Lagrangian of this optimization,
\bea
...
\eea
Using \eqref{eq:bethe_entorpy_information} for the Bethe entropy and remembering the derivative of $x\log\frac{x}{a}$ is $\log\frac{x}{a}+1$ the derivatives compare to zero give,
\bea
\log{\tau_i(x_i)} &=& \theta_i(x_i)+ \sum_{j \in \nei{i}} \lambda_{j \to i,x_i}-1+\lambda_i\\
...
This result give us some insight to the result BP may return and to how it should be used.
we mention before that there could be more than one fix point to BP.
This is now clear.
Local minima of the Bethe free energy is a stable fix point of BP.
