...
The classic way of understanding Pseudo-Likelihood (PL)\cite{besag1975statistical} is to see it as a way to approximate the likelihood.
...
\bea
p(x_k |\xx_{\setminus k};\thetav) &=& \frac{p(\xx;\thetav)}{p(\xx_{\setminus k};\thetav)}\\
...
Another view point of PL is just preforming ML on the conditional marginals, we are looking for the parameters that maximize the conditional probability for each vertex separately.
The derivative of the singleton parameter is the same.
The pairwise change so it include only one of the summation.
Since in this view the parameters distribute Normally, we can just take the average of the two approximation to the parameters.
Now all the characteristic of ML can be apply here.
Note that since we still need to approximate larger set - we still have slower convergence.
Moreover, since it directly dependent on its neighborhood a wrong neighborhood (wrong graph structure) is likely to have more impact than in ML ().
