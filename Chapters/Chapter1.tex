...
For example, we want to know what is the minimum set of variables that can explain specific variable - which verties are its neighbors.
Or which variable has the greatest effect on this variable etc.
...
Our objective is that when inferring the digit from an image we will have as answer the correct digit.
...
I will give a short survey on learning for reconstruction of the exact model.
...
There was a lot of research on the time and sample complexity of this problem.
...
While in other models (where the cardinality is greater than $|\cX|<2$ it is not known how to express this quantities.
Hence bound on non-binary models ignore these two factors\footnote{In proving upper bounds some conditions on the probability instead of the model is given. In my opinion the above two parameters hide there as well see \cite{bresler2008reconstruction}}.

The sample complexity bounds are $N > \Omega(d_{max} \log p)$ and $N > \Omega(\max\{d_{max}^2, \lambda^{-2}\}log p) $ to the general case and the Ising model respectively. This mean that we need at least $N$ samples in order to guarantee recovery of $\exact{E}$ in probability more that $0.5$.
Algorithm for the general case with sample complexity bounds need to assume something on the probability (see discussion above) hence I will give upper bounds only on the Ising model.
In the Ising model if $N > O(\max\{d^2, \lambda^2\}d\log p)$ than in high probability the exact graph $\exact{E}$ can be recover see\cite{santhanam2012information}.
