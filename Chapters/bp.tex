...
on tasks such as decoding error correcting codes \atodo{cite} and machine vision applications \atodo{cite}.
...
The resulting approximation for the marginals are given by:\atodo{No need for curly braces in exponent}
\bean
...
\eean
Note that $\tau_i,\tau_{ij}$ need not correspond to marginals of any distribution $P(\xx)$. In other words, they may be outside the marginal polytope.\atodo{cite yedidia for an example of this} Thus, they are often referred to as ``pseudo-marginals'', a term we will also use here.
