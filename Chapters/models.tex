...
%%U
\red{For example, in \cite{richardson2001design} a code which approaches capacity is suggested,where the decoding is achieved by Belief Propagation - an approximate algorithm.
 In \cite{decelle2011inference} the same algorithm was used for community detection in the stochastic block model, to achieve an optimal algorithm in cases where the detection is feasible.}
%%
