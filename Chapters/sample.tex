...
\begin{array}{lr}
0 & \xx' \notin \mathcal{D}^k(\xx)\\
\frac{(N-k)!k!}{N!}\frac{P(\xx')}{\sum_{\hat{\xx} \in \mathcal{D}(\xx,\cI)}P(\hat{\xx})} & \xx' \in \mathcal{D}^k(\xx), \xx \neq \xx'\\
\frac{(N-k)!k!}{N!}\sum_{\cI, |\cI|=k}\frac{P(\xx)}{\sum_{\hat{\xx} \in \mathcal{D}(\xx,\cI)}P(\hat{\xx})}& \xx = \xx'
\end{array} \right.
\ee
