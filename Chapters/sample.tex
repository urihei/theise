...
%%U
\red{In our case, each state is an assignment $\xx$, and the transition matrix describes the probability to move from state $\xx$ to state $\xx'$.
...
\red{If it is so, traversing the chain long enough is equivalent to sampling from $P(\xx)$.}
...
Sampling this Markov chain is easy; being in state $\xx$, a possible next state $\xx'$ is chosen by the distribution $Q(\xx' |\xx)$, and is accepted with probability  $\frac{Q(\xx|\xx')P(\xx')}{Q(\xx'|\xx)P(\xx)}$.
Hence, only $\frac{P(\xx)}{P(\xx')} $ is needed to be computed , and therefore the partition function is canceled, and the quantity is easy to calculate.}
%%
