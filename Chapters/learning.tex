...
\label{sec:learning}
\red{Learning GM is usually divided into two different tasks, structure learning and parameter learning.
  Structure learning attempts to find the dependency structure of the distribution,
  while in parameter learning, the exact value of each parameter is of interest \footnote{Structure learning can be seen as a sub-task of parameters learning. In the pairwise model, in specific parametrization, if a parameter is zero, then no edge exists between the corresponding two variables. So the structure may deduced by learning the parameters of the complete graph. Having the parameters, the edges are the the ones that the corresponding parameters of which are far from zero.} and usually the model structure is given.
  Evaluation of the two tasks is different as well.
...
\red{
  ...
  An objective is called consist, if given an infinite number of samples (and the correct structure), the maximizer of the objective $\thetav$ satisfies $P(\xx;\thetav) = P(\xx;\exact{\thetav})$.
