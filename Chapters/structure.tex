...
\footnote{\red{The definition of sample complexity in structure learning is as follow. Let $\nvars$ be the number of variables, $E$ be the result of the algorithm for a data-set of size $\nsamples$ and $\exact{E}$ be the true edge set. The algorithm has sample complexity of $g$, if it guarantee to return $E^*$ with probability grater than $1-\delta$ if the number of samples $\nvars$ is bigger that $O(g\log\frac{1}{\delta})$}.}
%%
