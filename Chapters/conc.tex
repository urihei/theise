...
\red{
  ...
  For example in GM, the common way the learn parameters is to use gradient descent for optimizing the ML.
  This, however, ignores the fact that approximate inference is used to find the gradient - the marginals.
  In the first paper, we show the implications of using BP as the approximate inference.
  Another weak point of this scheme is the definition of the ``best'' model.
  The most likely model (the model that optimizes ML) is the usual choice in GM.
 However, in many cases we are not interested in the model itself, but rather in its use for inference.
  Hence, the definition of ``best'' must take this into account, being the motivation for the second paper.
  Selecting a correct hypothesis class is a crucial step in the learning process.
  A good hypothesis class is one that has enough expressive power to model complex functions - but not too much as to reach over-fitting.
  Another quality, that is sometimes overlooked, is that it could be learned efficiently.
  In the third paper, the deep networks hypothesis class, that has been found to possess the first quality, is extended to allow efficient learning.
  I will now give a short summery for each of these papers. 
}
